...
def clean_text(text):
    ...
    # Remove MCQ bullets / answer circles / squares
    bullet_chars = [
        '●','○','◉','◎','◯','⬤','•','·',
        '■','□','▪','▫','▸','▹','►','▻',
        '✓','✔','✗','✘','→','⟶','➔','➜',
...
def extract():
    ...
    try:
        ...
        if not text.strip():
            return jsonify({
                'error': (
                    ...
                    'This PDF is likely scanned/image-based. '
                    'Text extraction only works on digital (born-digital) PDFs.'
                )
            }), 422
        ...
        # Build response with explicit headers so every browser triggers download
        ...
        return jsonify({'error': f'Server error while processing: {str(e)}'}), 500
